...
def Filter_Reads(reads_to_exclude, reads_to_keep, inpath, out_h):
    ...
    handle = sp.Popen((cmd, inpath), bufsize=8192, stdout=sp.PIPE, universal_newlines=True).stdout
    line = handle.readline()
    num_reads = 0
    while line:
...
if __name__ == '__main__':
    ...
    handle = sp.Popen((cmd, _args.k), bufsize=8192, stdout=sp.PIPE, universal_newlines=True).stdout
    line = handle.readline()
    while line:
    ...
    loginfo('Retaining {} reads ({} taxa specified).'.format(len(reads_to_keep) if reads_to_keep else 'all other',
                                                             len(_args.r)))

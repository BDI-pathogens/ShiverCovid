...
def check_file_exists(my_file):
    if not os.path.isfile(my_file):
        raise argparse.ArgumentTypeError(my_file + ' does not exist or is not a file.')
    return my_file
...
parser.add_argument('FastaFile', type=check_file_exists, nargs="+")
    ...
    for file_ in args.FastaFile:
        for seq in SeqIO.parse(open(file_), 'fasta'):
            ...
            if args.longest_gap:
                ...
                seq_lengths.append([seq.id, longest_gap_length])
                if args.first_seq_only:
                    break
                continue
            ...
            if args.fragments:
                frag_lengths = [len(frag) for frag in seq.seq.split('?') if len(frag) > 0]
                frag_lengths = sorted(frag_lengths, reverse=True)
                frag_lengths.append(0)
                seq_lengths.append([seq.id] + frag_lengths)
            else:
                seq_lengths.append([seq.id, len(seq.seq)])
            if args.first_seq_only:
                break

    for data in seq_lengths:
